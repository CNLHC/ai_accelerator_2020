...
pub fn load() -> ((NdArray, NdArray), (NdArray, NdArray)) {
    let (train_x, num_image_train): (Vec<f32>, usize) = load_images("data/train-images-idx3-ubyte");
    let (train_y, num_label_train): (Vec<f32>, usize) = load_labels("data/train-labels-idx1-ubyte");
    let (test_x, num_image_test): (Vec<f32>, usize) = load_images("data/t10k-images-idx3-ubyte");
    let (test_y, num_label_test): (Vec<f32>, usize) = load_labels("data/t10k-labels-idx1-ubyte");
...
fn load_images<P: AsRef<Path>>(path: P) -> (Vec<f32>, usize) {
    let ref mut buf_reader = io::BufReader::new(File::open(path).expect("no data found"));
